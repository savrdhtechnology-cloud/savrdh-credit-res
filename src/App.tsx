...
export default function App() {
  ...
  useEffect(() => {
    const handleUrlChange = () => {
      ...
      const hash = window.location.hash.toLowerCase();
      const pathname = window.location.pathname.toLowerCase();
      if (search.includes("admin") || search.includes("page=admin") || hash.includes("admin") || pathname.includes("admin")) {
        setIsAdminMode(true);
      }
    };
  ...
  const [userProfile, setUserProfile] = useState<UserProfile>({
    fullName: "", mobile: "", email: "", isMobileVerified: false, isEmailVerified: false, biometricEnabled: false,
  });
  ...
  const handleBiometricLogin = () => {
    if (crmLead && paymentDetails) setCurrentStep("DASHBOARD");
    else if (userProfile.fullName && userProfile.isMobileVerified) setCurrentStep("KYC");
  ...
  const handleKycComplete = (kyc: KYCData) => {
    setKycData(kyc);
    if (kyc.fetchedProfile?.name) setUserProfile((prev) => ({ ...prev, fullName: kyc.fetchedProfile!.name }));
    setCurrentStep("CREDIT_REPORT");
  };
  const handleProceedToAnalysis = (report: CreditBureauReport) => {
    setCreditReport(report);
    if (report.verifiedProfile?.matchedName) setUserProfile((prev) => ({ ...prev, fullName: report.verifiedProfile!.matchedName }));
    if (report.verifiedProfile?.matchedPan) setKycData((prev) => ({ ...prev, panNumber: report.verifiedProfile!.matchedPan }));
  ...
  const handleResetFlow = () => {
    ...
    setUserProfile({ fullName: "", mobile: "", email: "", isMobileVerified: false, isEmailVerified: false, biometricEnabled: false });
    setKycData({ aadhaarNumber: "", maskedAadhaar: "", panNumber: "", isVerified: false });
    setPaymentDetails(null); setCrmLead(null);
  };
  ...
  return (
    <MobileContainer currentStep={currentStep} onResetFlow={handleResetFlow} onOpenAdminCRM={() => setIsAdminMode(true)} onOpenCibilAnalyzer={() => setActiveStandaloneTool("CIBIL_ANALYZER")} onOpenLoanAnalyzer={() => setActiveStandaloneTool("LOAN_ANALYZER")}>
      ...
      {activeStandaloneTool === "CIBIL_ANALYZER" && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
          <div className="w-full max-w-4xl max-h-[92vh] overflow-y-auto rounded-3xl bg-navy-950 border border-amber-500/30 shadow-2xl shadow-black/90 p-4 sm:p-6 relative">
            <button onClick={() => setActiveStandaloneTool(null)} className="absolute top-4 right-4 p-2 rounded-xl bg-navy-900 border border-slate-700 text-slate-400 hover:text-white hover:border-amber-500/50 transition-all z-10" title="Close Tool"><X className="w-5 h-5" /></button>
            <CibilReportAnalyzer initialReport={creditReport} onClose={() => setActiveStandaloneTool(null)} onApplyToApp={(newReport) => {
              setCreditReport(newReport);
              const verified = newReport.verifiedProfile;
              if (verified?.matchedName?.trim()) setUserProfile((prev) => ({ ...prev, fullName: verified.matchedName.trim() }));
              if (verified?.matchedPan?.trim()) setKycData((prev) => ({ ...prev, panNumber: verified.matchedPan.trim().toUpperCase() }));
              if (currentStep === "SPLASH" || currentStep === "REGISTRATION" || currentStep === "KYC") setCurrentStep("CREDIT_REPORT");
      ...
      {activeStandaloneTool === "LOAN_ANALYZER" && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
          <div className="w-full max-w-4xl max-h-[92vh] overflow-y-auto rounded-3xl bg-navy-950 border border-indigo-500/30 shadow-2xl shadow-black/90 p-4 sm:p-6 relative">
            <button onClick={() => setActiveStandaloneTool(null)} className="absolute top-4 right-4 p-2 rounded-xl bg-navy-900 border border-slate-700 text-slate-400 hover:text-white hover:border-indigo-500/50 transition-all z-10" title="Close Tool"><X className="w-5 h-5" /></button>
            <LoanStatementAnalyzer onClose={() => setActiveStandaloneTool(null)} isStandalone={true} />
          </div>
        </div>
      )}
      ...
      <div className="fixed bottom-3 right-3 z-40"><button onClick={() => { if (adminUser) setIsAdminMode(true); else setIsAdminLoginOpen(true); }} className="py-2 px-3.5 rounded-2xl bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-600 hover:from-amber-400 hover:to-yellow-500 text-navy-950 font-bold text-xs shadow-xl shadow-black/60 flex items-center gap-2 border border-amber-300/40 transition-all transform hover:scale-105 active:scale-95"><ShieldCheck className="w-4 h-4" /><span>Staff Admin CRM</span></button></div>
